<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.units.create') }}" class="px-4 py-2 bg-gray-900 text-white rounded">
                Tambah
            </a>
        </div>
    </x-slot>
        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="bg-white p-4 rounded shadow">
                <form method="GET" class="flex gap-2">
                    <input name="q" value="{{ $q }}" placeholder="Cari satuan..."
                    ...
                    <tbody>
                        @forelse($units as $u)
                            <tr class="border-t">
                                ...
                                <td class="p-3">{{ $u->symbol ?? '-' }}</td>
                                <td class="p-3">{{ $u->is_active ? 'Ya' : 'Tidak' }}</td>
                                <td class="p-3 text-right space-x-2">
                                    <a class="text-blue-600" href="{{ route('admin.units.edit', $u) }}">Edit</a>
                        ...
                        @empty
                            <tr>
                                <td class="p-3" colspan="4">Tidak ada data.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
